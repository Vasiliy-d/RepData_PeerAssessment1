...
Mean total number of steps taken per day is **`r format(meanSteps)`**.  
Median total number of steps taken per day is **`r format(medianSteps)`**.
...
**`r intervalMax`th** interval contains maximum maximum number of steps across all days in the dataset
...
Mean total number of steps taken per day after data input is **`r format(meanSteps2)`**. Median total number of steps taken per day after data input is **`r format(medianSteps2)`**.  
...
```{r create.new.variable, results = "hide"}
...
```{r plot.daily.activity.weekday, fig.height = 10}
